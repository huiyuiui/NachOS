...
int main(int argc, char **argv)
{
    ...
    if (recurRemoveFileName != NULL) {
        kernel->fileSystem->RecurRemove(recurRemoveFileName);
    }
    ...
    {
        // MP4 mod tag
        kernel->fileSystem->CreateSubDir(createDirectoryName);
    }
    if(countHeader){
        kernel->fileSystem->CountHeaderSize(countFileName);
    }
