...
{
    approx_burst = 0;      // approximate burst time
    last_approx_burst = 0; // last approximate burst time
    start_time = 0;        // time when start running
    end_time = 0;          // time when ended running
    total_ready_time = 0;  // total time waiting in ready queue
    insert_ready_time = 0; // the time when thread enters ready queue
    remain_burst = 0;
    true_burst = 0;
    cur_check_time = 0;
    priority = kernel->priorities[threadID];
    ID = threadID;
    name = threadName;
    stackTop = NULL;
    stack = NULL;
...
void Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    scheduler->PutToReady(this); // ReadyToRun assumes that interrupts
                                 // are disabled!
...
void Thread::Yield()
{
    ...
    nextThread = kernel->scheduler->ScheduleNext();
    if (nextThread != NULL)
    {
        this->EndRunning();
        this->UpdateBurst_run_to_ready();
        kernel->scheduler->PutToReady(this);
...
void Thread::Sleep(bool finishing)
{
    ...
    status = BLOCKED;
    this->EndRunning();
    this->UpdateBurst_run_to_wait();

    // cout << "debug Thread::Sleep " << name << "wait for Idle\n";
    while ((nextThread = kernel->scheduler->ScheduleNext()) == NULL)
    {
...
void Thread::StartReady()
{
    this->insert_ready_time = kernel->stats->totalTicks;
}

void Thread::StartRunning()
{
    this->start_time = kernel->stats->totalTicks;
    this->cur_check_time = kernel->stats->totalTicks;
    this->total_ready_time = 0;
}

void Thread::EndRunning()
{
    this->end_time = kernel->stats->totalTicks;
}

void Thread::UpdateBurst_run_to_wait()
{
    this->true_burst += this->end_time - this->cur_check_time;
    this->remain_burst -= this->end_time - this->cur_check_time;
    double approx_burst = 0.5 * this->true_burst + 0.5 * this->approx_burst;
    this->last_approx_burst = this->approx_burst;
    this->approx_burst = approx_burst;
    DEBUG(dbgScheduler, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from: [" << this->last_approx_burst << "], add [" << this->true_burst << "], to [" << this->approx_burst << "]");
}

void Thread::UpdateBurst_run_to_ready()
{
    this->true_burst += this->end_time - this->cur_check_time;
    this->remain_burst -= this->end_time - this->cur_check_time;
}

void Thread::RemainBurst_update()
{
    this->true_burst += kernel->stats->totalTicks - this->cur_check_time;
    this->remain_burst -= kernel->stats->totalTicks - this->cur_check_time;
    this->cur_check_time = kernel->stats->totalTicks;
}
