...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        if (strcmp(argv[i], "-ep") ==0)
        {
            ...
            ASSERT(priority_str[0]>='0' && priority_str[0]<='9');
            int priority_num = atoi(priority_str);
            ASSERT(priority_num>=0 && priority_num<=149);
            priorities[execfileNum] = priority_num;
        }
